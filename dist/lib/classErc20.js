...
class Erc20 {
  constructor(db) {
    this.db = db;
    this.tokenCollection = db.collection(_config2.default.erc20.tokenCollection);
    this.tokenList = [];
    this.tokens = {};
        ...
        if (err) {
          ...
          for (let token of docs) {
            ...
            if (!this.tokens[address]) {
              let collection = db.collection(_config2.default.erc20.dbPrefix + token.address);
    ...
    this.actions = {
      ...
      getEvents: params => {
        ...
        return this.paginator(query, params).then(totals => {
          return this.db.find(query).sort({ timestamp: -1 }).skip(0).limit(50).toArray().then(data => {
            return { totals, data };
          });
        });
      },
      getAccount: params => {
        ...
        return this.db.findOne({ _id: account }).then(balance => {
          return this.db.find({ $or: [{ 'args._from': account }, { 'args._to': account }] }).sort({ timestamp: -1 }).skip(offset).limit(50).toArray().then(account => {
      ...
      getAccounts: params => {
        ...
        return this.paginator(query, params).then(totals => {
          return this.db.find(query).sort({ _id: 1 }).skip(offset).limit(50).toArray().then(data => {
            return { totals, data };
          });
        });
      },
