"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.Router = Router;
var _ServiceClient = require("./Service/ServiceClient");

function Router({ service, log } = {}) {
  log = log || console;
  service = service || {};
  ...
  const addClient = async ({ uri, name }) => {
    try {
      ...
      const res = await (0, _ServiceClient.Client)(uri).catch(err => {
        if (err.code === 14) return Promise.resolve();else
        throw err;
      });
      if (!res) return;
