...
if (web3.isConnected()) console.log("Web3 connection established");else throw "No connection";
    ...
    if (err) {
      ...
      for (var l in log) {
        ...
        new DAOCreatedToken(newToken).save(function (err, token, count) {
          ...
          } else console.log('DB successfully written for tx ' + token.transactionHash);
        });
      }
    }
  });
    ...
    if (err) {
      ...
      for (var l in log) {
        ...
        new DAOTransferToken(newToken).save(function (err, token, count) {
          ...
          } else console.log('DB successfully written for tx ' + log[l].transactionHash);
        });
      }
    }
  });
...
var bulkTimeUpdate = function (bulk, callback) {
  ...
  bulk.execute(function (err, result) {
    if (err) console.error(err);else console.log(result.toJSON());
  });
};
