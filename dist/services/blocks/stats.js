...
var getStats = function (web3, blockNumber, nextBlock, endBlock) {
    if (blockNumber <= endBlock) process.exit(9);
    ...
    if (web3.isConnected()) {
        ...
        web3.eth.getBlock(blockNumber, true, function (error, blockData) {
            if (error) {
                ...
                if (nextBlock) checkBlockDBExistsThenWrite(web3, blockData, nextBlock.timestamp);else checkBlockDBExistsThenWrite(web3, blockData, parseInt(Date.now() / 1000));
            }
        });
    } else {
